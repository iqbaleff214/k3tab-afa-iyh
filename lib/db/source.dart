...
List<Engine> engines = [
  ...
  Engine('Bearing', Code.bearing, Category.powertrain, true, [
    ...
    '[true, false, false, false, true, false, false, false, false, false, false, false]': results[ResultCode.goodCondition],
    '[true]': results[ResultCode.contactStressFatigue]?.copyWith(causes: ['Penerapan beban terlalu berlebihan', 'Setelan pemasangan terlalu ketat', 'Setelan pemasangan terlalu longgar', 'Shaft terlalu besar atau housing terlalu kecil', 'Tidak lurus/sejajar (misalignment)', 'Viskositas pelumasan rendah']),
    '[true]': results[ResultCode.plasticYealding]?.copyWith(causes: ['Penggunaan oli dengan viskositas terlalu rendah', 'Temperature kerja tinggi (suhu udara luar sangat tinggi)', 'Jumlah oli sangat sedikit', 'Terlalu banyak oli atau grease menyebabkan teraduk-aduk (churning)', 'beban tinggi', 'Salah aplikasi', 'Misalignment', 'Misadjustment', 'Pengepasan dimensi yang salah']),
    '[true]': results[ResultCode.adhesiveWear]?.copyWith(causes: ['Kekurangan supply oli', 'Cage aus berlebihan atau pecah', 'Temperature kerja tinggi']),
    '[true]': results[ResultCode.abrasiveWear]?.copyWith(causes: ['Kontaminan dalam oli']),
    '[true]': results[ResultCode.corrosionWear]?.copyWith(causes: ['Bearing tidak dilindungi (dengan oli, grease atau pelindung lainnya) setelah pembongkaran dilakukan', 'Bearing tidak disimpan ditempat yang kering', 'Embun masuk dalam sistem pelumasan']),
    '[true]': results[ResultCode.frettingCorrosion]?.copyWith(causes: ['Shaft terlalu kecil', 'Housing bore terlalu besar', 'Material asing tersangkut diantara roller dan raceway', 'Beban thrust terlalu besar']),
    '[true]': results[ResultCode.electricalFitting]?.copyWith(causes: ['Arus tinggi yang terjadi sementara bearing tidak berputar (umumnya menyebabkan pancaran bunga matahari)', 'Arus rendah yang terjadi pada masa waktu yang panjang sementara bearing sedang berputar']),
    '[true]': results[ResultCode.damageDueInstallation]?.copyWith(causes: ['Salah dalam prosedur pemasangan dan pelepasan']),
    '[true]': results[ResultCode.ductileFracture]?.copyWith(causes: ['Beban kejut/overload', 'Misalignment']),
    '[true]': results[ResultCode.fatigueFracture]?.copyWith(causes: ['Cyclic overload']),
    '[true]': results[ResultCode.brittleFracture]?.copyWith(causes: ['Beban kejut/overload', 'Misalignment']),
  }),
